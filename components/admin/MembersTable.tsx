...
import { MoreVertical, Edit, Trash2, Eye, CheckCircle, XCircle, Search, Filter, Download } from 'lucide-react';
...
const fetcher = (url: string) => fetch(url).then((res) => res.json()).then((data) => {
  const users = data.getAllRegistrations || [];
...
export default function MembersTable({ statusFilter }: { statusFilter?: string }) {
  const { data, error, isLoading, mutate } = useSWR(`${process.env.NEXT_PUBLIC_API_URL}/all-registrations`, fetcher);
  ...
  const handleDelete = (id: string | number) => {
    if (window.confirm("Are you sure you want to delete this member? This action cannot be undone.")) {
      mutate(members.filter(m => m.id !== id), false);
    }
  };
  ...
  const handleSave = async (updatedMember: Member) => {
    ...
    if (isNew) {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/member-registration`, {
          method: 'POST',
  ...
  const handleDownload = () => {
    const headers = ["Full Name", "Gender", "Phone", "Occupation", "Status", "Joined Date"];
    const csvContent = [
      headers.join(","),
      ...filteredMembers.map(m => 
        [m.fullName, m.gender, m.phoneNumber, m.occupation, m.status, m.joinedDate].map(field => `"${field}"`).join(",")
      )
    ].join("\n");
  ...
  if (error) return <div className="p-6 text-center text-red-500">Failed to load members.</div>;
  if (isLoading) return <div className="p-6 text-center text-gray-500">Loading members...</div>;
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-6 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
          <h2 className="text-lg font-bold text-gray-900">
            ...
            <span className="ml-2 text-sm font-normal text-gray-500">({filteredMembers.length})</span>
          </h2>
